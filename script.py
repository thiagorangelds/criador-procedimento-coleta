...
def gerar_resposta_gemini_com_fallback(prompt: str, api_key: str, modelos: list, max_retries: int, logger: logging.Logger) -> str:
...
'''
        ...
        try:
            resposta_completa = gerar_resposta_gemini_com_fallback(prompt, API_KEY, MODELOS, RETRY, logger)
            ...
            if SEPARADOR_CATALOGO in resposta_completa:
                ...
                catalogo = catalogo_com_marcador.replace(MARCADOR_CATALOGO, "").strip()
                
            elif MARCADOR_CATALOGO in resposta_completa:
                partes = resposta_completa.split(MARCADOR_CATALOGO, 1)
                procedimento = partes[0]
                catalogo = f"{MARCADOR_CATALOGO}\n{partes[1].strip()}"
